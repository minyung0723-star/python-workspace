...
for hit in result1["hits"]["hits"]:
    print(f"- {hit['_source']['title']} ({hit['_source']['year']})")
...
for hit in result2["hits"]["hits"]:
    print(f"- {hit['_source']['title']} | {hit['_source']['genre']} | {hit['_source']['rating']}")
...
result3 = es.get(index="movies", id=4)
print(f"수정 후: {result3['_source']}")
# TODO: 여기를 채우세요
