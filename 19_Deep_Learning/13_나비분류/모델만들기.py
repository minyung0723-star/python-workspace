...
def 각폴더별_데이터개수확인(경로):
    """
    train / test / valid 폴더 안에
    각각 몇 장의 사진이 있는지 출력하는 함수
    """
    for 분류 in ['train', 'test', 'valid']:
        분류경로 = 경로 / 분류
        전체 = 0
        종류수 = 0
        for 종폴더 in 분류경로.iterdir():
            if 종폴더.is_dir():
                개수 = len(list(종폴더.glob("*.jpg")))
                전체 += 개수
                종류수 += 1
        print(f"{분류} : {종류수}종류 / {전체:,}장")
...
def 깨진파일제거(경로):
    """
    train / test / valid 폴더 안의 jpg 파일을 열어보고
...
# 과제 3 : 데이터 불러오기 함수
# =============================================
def 데이터불러오기(경로, 이미지크기=(224, 224), 배치=32):
    """
    train / valid 폴더에서 데이터를 불러오고 정규화하는 함수
    나비 데이터는 폴더가 이미 train/valid 로 나뉘어 있어요.
    validation_split 필요없음
    """
...
# 과제 4 : 모델 만들기 함수
# =============================================
def 모델만들기(이미지크기=(224, 224)):
    """
    CNN 모델을 만들고 컴파일까지 하는 함수
    나비 100종류 → 마지막 Dense(100), softmax 사용
    """
    모델 = Sequential([
        ...
        Dense(100, activation='softmax'),   # 나비 100종류
    ])
    모델.compile(
        optimizer='adam',
...
# 과제 5 : 모델 저장 함수
# =============================================
def 모델저장(모델, 폴더='models', 파일명='butterfly_model.keras'):
