...
model = tf.keras.Sequential([  # 뇌만들기 시작!
    ...
    # 2D = input_shape(28, 28)
    # 3D = input_shape(28, 28, 3)과 같이 작성 컬러 이미지의 경우 3D 이다 왜냐하면 빨강 초록 파랑으로 이루어진 데이터이므로 3개 데이터가 필요
...
- sigmoid = 어떤 숫자든 0~1 사이로 교체
  아주 큰 숫자 0.99 보통 0.5 아주 작은 숫자 0.01
  스팸메일이나, 암이냐, 아니냐, 합격이냐 불합격이냐 같은 둘 중 하나 고르는 모델의
  마지막 층 사용
...
- 마찬가지로 한 번에 처리하는 샘플 수가 많을 수록 GPU 메모리 소모가 크다.
step_per_epoch
- 에폭 당 배치 수
validation_split
- 검증 데이터 비율
