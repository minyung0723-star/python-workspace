...
플레이어_y = 530
플레이어_크기 = 35
속도 = 7       # TODO 1: 플레이어 이동 속도

# 똥 설정
똥목록 = []
똥속도 = 5     # TODO 2: 똥 떨어지는 속도
타이머 = 0
...
while 실행중:
    for 이벤트 in pygame.event.get():
        ...
        if 이벤트.type == pygame.KEYDOWN:
            if 게임오버 and 이벤트.key == pygame.K_r:
                똥목록.clear()
                플레이어_x = 400
                점수, 똥속도, 게임오버 = 0, 5, False   # TODO 3: 재시작 초기 똥속도 (TODO 2랑 같은 숫자)
    ...
    # 화면 그리기 (위치 수정: if not 게임오버 밖으로 이동하여 매 프레임 실행)
...
# 프로그램 종료 (위치 수정: while 루프가 완전히 끝난 뒤 실행)
pygame.quit()