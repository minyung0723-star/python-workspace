...
def 운동칼로리_모델만들기():
    ...
    with open(f'models/excercise_calories_{r2_score(y_test,y_pred):.4f}.pkl','wb') as f:
        pickle.dump(model,f)
    print(f'models/excercise_calories_{r2_score(y_test,y_pred):.4f}.pkl 저장 완료')
...
def 광고매출_모델만들기():
    ...
    print(f"R² : {r2_score(y_test,y_pred):.4f}")
    # 광고 비용으로 매출 예측
    # 소셜에 광고비용 basic ~ premium basic 예상 기대효과 수익 예측 추천
    with open(f'models/ad_sales_{r2_score(y_test,y_pred):.4f}.pkl','wb') as f:
        pickle.dump(model,f)
    print(f'models/ad_sales_{r2_score(y_test,y_pred):.4f}.pkl 저장 완료')
